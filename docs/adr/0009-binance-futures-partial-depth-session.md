...
- Use one combined WebSocket connection with `<symbol>@depth20@100ms` and
  `<symbol>@aggTrade` for every configured market.
- Treat each partial-depth message as a complete top-20 snapshot, not as a
...
Binance provides a higher-frequency top-20 L2 view and aggregate trades while
preserving the existing synchronous runtime and normalized event contract.
Levels beyond the top 20 are not captured. Switching this adapter to full
